...
const formSchema = z.object({
  file: z.instanceof(File, { message: `Please select a ${TERMS.file.toLowerCase()}` }),
...
export default function UploadPage() {
  ...
  const structuredData = {
    ...
    operatingSystem: "All",
    description: "Upload and share files securely with time-limited and download-limited access.",
    offers: {
      "@type": "Offer",
      price: "0",
      priceCurrency: "USD",
  ...
  const uploadMutation = useMutation({
    mutationFn: async (values: z.infer<typeof formSchema>) => {
      if (!user) throw new Error(`You must be logged in to ${TERMS.upload.toLowerCase()} ${TERMS.file.toLowerCase()}s`)
          ...
          <div className="mx-auto max-w-2xl">
            <div className="mb-8">
              ...
              <p className="text-muted-foreground">
                {TERMS.upload} a {TERMS.file.toLowerCase()} to {TERMS.transport.toLowerCase()} securely with
                time-limited and {TERMS.views.toLowerCase()}-limited access.
              </p>
            </div>
            ...
            {shareLink ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="text-center space-y-2">
                      <h2 className="text-xl font-semibold">
                        {TERMS.file} {TERMS.upload.toLowerCase()}ed successfully!
                      </h2>
                      <p className="text-muted-foreground">
                        Your {TERMS.file.toLowerCase()} has been {TERMS.upload.toLowerCase()}ed and is ready to{" "}
                        {TERMS.transport.toLowerCase()}. Copy the {TERMS.link.toLowerCase()} below to{" "}
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                            ...
                            {uploadedFile ? (
                              ...
                              <div className="flex flex-col items-center gap-2">
                                ...
                                <p className="text-sm font-medium">
                                  Drag and drop your {TERMS.file.toLowerCase()} here, or click to browse
                                </p>
                                <p className="text-xs text-muted-foreground">Supports any file format</p>
                              </div>
                            )}
                          </div>
                        </FormControl>
                  ...
                  <div className="grid gap-6 sm:grid-cols-2">
                          ...
                          <FormControl>
                            <div className="space-y-2">
                              ...
                              <div className="flex justify-between text-xs text-muted-foreground">
                                ...
                                <span>
                                  {Math.round(field.value / 24)} {Math.round(field.value / 24) === 1 ? "day" : "days"}
                                </span>
                              </div>
                            </div>
                          </FormControl>
                          <FormDescription>
                            The {TERMS.link.toLowerCase()} will expire after this time period
                          </FormDescription>
                        </FormItem>
                      )}
                    />
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="flex items-center gap-2">
                            <Eye className="h-4 w-4" /> Max {TERMS.views.toLowerCase()}
                          </FormLabel>
                          <FormControl>
                            <div className="space-y-2">
                              <Slider
                          ...
                          <FormDescription>
                            The {TERMS.link.toLowerCase()} will expire after this many {TERMS.views.toLowerCase()}
                          </FormDescription>
                        </FormItem>
                      )}
                    />
                  ...
                  <Button type="submit" className="w-full" disabled={uploadMutation.isPending || !uploadedFile}>
